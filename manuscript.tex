...
the range of the linear MCI growth rates. This juxtaposition is reasonable since
nonlinear PIC code simulations indicate that all growing modes stop when the
fastest growing mode becomes nonlinear at time $\tau$. Under the assumption
that all modes start growing in unison from identical seed amplitudes then the
$log$ of the amplitude of a mode with growth rate $\gamma$ is proportional to
$\gamma \tau$, allowing for a comparison with observations also on a log scale.
